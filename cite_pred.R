...
cite_pred<-function(data, date, group, unigram, threshhold){
  ...
  count <- data %>%
    mutate(
      word_freq = str_count(string = data$Abstract, pattern = unigram1),
    ...
    group_by(decade) %>%
    nest() %>%
    mutate(Model = map(data, cited_by),
  ...
  temp2 <-
    left_join(
      ...
      temp %>% filter(term == "Terms Not Present") %>% select(decade, estimate) %>%
        rename(Mean = estimate)
    ) %>%
    mutate(`Estimate With` = exp(Mean + estimate),
           `Estimate Without` = exp(Mean)) %>%
    select(
      Decade = decade,
      `Estimate Without`,
      `Estimate With`,
